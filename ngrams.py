...
def tfidf_word_ngrams(corpus, min_n, max_n, num_pairs):
    ...
    tagged_pairs = t_w_ngrams_vec[np.array(list(df['text_id'][:num_pairs]))]
    cos = [spatial.distance.pdist(pair, metric='cosine') for pair in tagged_pairs]

    return np.concatenate(cos, axis = 0)

def tfidf_char_ngrams(corpus, min_n, max_n, num_pairs):
    ...
    tagged_pairs = t_c_ngrams_vec[np.array(list(df['text_id'][:num_pairs]))]
    cos = [spatial.distance.pdist(pair, metric='cosine') for pair in tagged_pairs]
